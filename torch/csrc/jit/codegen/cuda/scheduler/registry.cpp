...
class SchedulerTopologyChecker {
 public:
  ...
  size_t SchedulerRuntimeInfo::getMaxVectorizableWidth(TensorView* tv) {
    ...
    // Appears after reductions the reduction domain often has a contiguity
    // entry. This only matters if the result of the reduction is an output
  ...
  std::vector<TransposeOp*> findTransposeOps(Fusion* fusion) {
    ...
    return std::vector<TransposeOp*>(
        transpose_ops.begin(), transpose_ops.end());
  }
  ...
  //!        contribute to dynamic shape latency so `data_cache` should be used
  //!        as much as possible to save re-computation.
  //!
  //!     3. schedule(fusion):
  //!
  //!        This function will be called when compiling a kernel. It should
  //!        apply scheduling to the given fusion
  ...
  class ReductionScheduler : public SchedulerEntry {
   public:
    ...
    static bool canScheduleCompileTime(Fusion* fusion) {
      ...
      if (ir_utils::getReductionOps(fusion, true /* ignore_trivial */)
              .empty()) {
        scheduler_debug_utils::canScheduleRejectReason(
  ...
  class PersistentKernelScheduler : public SchedulerEntry {
   public:
    ...
    static bool canScheduleRunTime(
      ...
      // If there's a small iteration dimension but a large reduction dimension
      // it may not make sense to make a persistent kernel
      auto properties = scheduler_utils::getProperties(
      ...
      if (
        ...
        scheduler_debug_utils::canScheduleRejectReason(
            ScheduleHeuristic::Persistent,
            "unsupported cross grid persistence");
  ...
  bool checkCanSchedule(
      ...
      SchedulerRuntimeInfo& runtime_info,
      HeuristicSummary* data_cache = nullptr) {
    // If a data cache is given, the compile time part doesn't need to be
    // checked, since for all current use cases
    //  it has to pass all the compile time checks to create a data cache for
    //  this fusion.
    if (!data_cache) {
  ...
  HeuristicSummary::HeuristicSummary(
    ...
    switch (heuristic) {
      ...
      case ScheduleHeuristic::Persistent:
        ...
        PersistentKernelScheduler::canScheduleRunTime(

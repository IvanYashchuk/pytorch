...
IndexFromIdGraph getPredicateIndexingFromIdGraph(
  ...
  for (IterDomain* consumer_id :
    ...
    auto almost_exact_2_consumer_id_it =
        almost_exact_2_consumer_id.find(almost_exact_concrete_id);
    if (almost_exact_2_consumer_id_it == almost_exact_2_consumer_id.end()) {
      almost_exact_2_consumer_id_it =
          almost_exact_2_consumer_id
              .emplace(std::make_pair(
                  almost_exact_concrete_id,
                  VectorOfUniqueEntries<IterDomain*>()))
    ...
    auto& mapped_dims = almost_exact_2_consumer_id_it->second;
    mapped_dims.pushBack(consumer_id);
  }
  ...
  for (auto entry : indexing.indexMap()) {
    ...
    if (almost_exact_2_consumer_id.find(almost_exact_concrete_id) ==
        almost_exact_2_consumer_id.end()) {
      continue;
    }
    auto consumer_ids = almost_exact_2_consumer_id.at(almost_exact_concrete_id);

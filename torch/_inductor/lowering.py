...
def _boundaries_helper(tb: TensorBox) -> tuple[str, sympy.Expr, sympy.Expr, sympy.Expr]:
    # Calculate the exclusive storage extent for the boundaries tensor relative
    # to its first logical element. Use last-used-offset + 1 rather than
    # + stride[-1] so expanded zero-stride boundaries keep their aliased element
    # visible to the bucketize/searchsorted mask.
    size = tb.get_size()
    stride = tb.get_stride()
    max_offset = sum((s - 1) * st for s, st in zip(size, stride)) + 1
    return (
        tb.get_name(),
        size[-1],
        max_offset,

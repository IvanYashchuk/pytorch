...
from typing import Any, Callable, cast, ClassVar, Generic, NamedTuple, TYPE_CHECKING
...
_backend_inductor_meta_providers: dict[str, BackendInductorMetaProvider] = {}
BackendPostCompileHook = Callable[
    [str, dict[str, Any]], str | SequenceABC[str] | None
]
_backend_post_compile_hooks: dict[str, BackendPostCompileHook] = {}
...
def register_cuda_backend(
    name: str,
    device_scheduling: SchedulingConstructor,
) -> None:
    """
    Register a CUDA codegen backend selectable via ``config.cuda_backend``.

    This lets out-of-tree packages add CUDA backends without replacing the
...
def register_backend_kernel_launcher(
    name: str,
    ...
    if existing is not None and not _same_registered_symbol(existing, launcher):
        raise ValueError(f"Kernel launcher backend {name!r} is already registered")
    _backend_kernel_launchers[name] = launcher
...
def register_backend_inductor_meta_provider(
    name: str,
    provider: BackendInductorMetaProvider,
) -> None:
    """
    Register a backend-owned hook for generated-kernel ``inductor_meta``.

    The provider receives ``(kernel, inductor_meta)`` after the kernel body has
    been generated and before the heuristics decorator is rendered. It may
    mutate ``inductor_meta`` in place or return a dict of additional fields.
    """
    _validate_backend_name("inductor meta provider", name)
    existing = _backend_inductor_meta_providers.get(name)
    if existing is not None and not _same_registered_symbol(existing, provider):
        raise ValueError(
            f"Inductor meta provider backend {name!r} is already registered"
        )
    _backend_inductor_meta_providers[name] = provider


def get_backend_inductor_meta_provider(
    name: str,
) -> BackendInductorMetaProvider | None:
    _validate_backend_name("inductor meta provider", name)
    return _backend_inductor_meta_providers.get(name)


def apply_backend_inductor_meta_provider(
    kernel: Any,
    inductor_meta: dict[str, Any],
    backend: str | None = None,
) -> None:
    if backend is None:
        kernel_backend = getattr(kernel, "backend", None)
        backend = kernel_backend() if callable(kernel_backend) else get_current_backend()
    provider = get_backend_inductor_meta_provider(backend)
    if provider is None:
        return

    extra_meta = provider(kernel, inductor_meta)
    if extra_meta is None:
        return
    if not isinstance(extra_meta, dict):
        raise TypeError(
            f"Inductor meta provider backend {backend!r} must return a dict or None"
        )
    inductor_meta.update(extra_meta)


def register_backend_post_compile_hook(
    name: str,
    hook: BackendPostCompileHook,
) -> None:
    """
    Register generated-wrapper statements to run after async kernel compilation.

    The hook receives ``(kernel_name, inductor_meta)`` after a generated wrapper
    has called ``async_compile.wait(globals())``. It may return a Python source
    line, a sequence of Python source lines, or ``None``.
    """
    _validate_backend_name("post compile hook", name)
    existing = _backend_post_compile_hooks.get(name)
    if existing is not None and not _same_registered_symbol(existing, hook):
        raise ValueError(f"Post compile hook backend {name!r} is already registered")
    _backend_post_compile_hooks[name] = hook


def get_backend_post_compile_hook(name: str) -> BackendPostCompileHook | None:
    _validate_backend_name("post compile hook", name)
    return _backend_post_compile_hooks.get(name)


def get_backend_post_compile_hook_lines(
    kernel_name: str,
    ...
    hook = get_backend_post_compile_hook(backend)
    if hook is None:
        return ()

    lines = hook(kernel_name, inductor_meta)
    if lines is None:
        return ()
    if isinstance(lines, str):
        return (lines,)
    if not isinstance(lines, SequenceABC):
        raise TypeError(
            f"Post compile hook backend {backend!r} must return a string, "
            "a sequence of strings, or None"
        )
    normalized = tuple(lines)
    if not all(isinstance(line, str) for line in normalized):
        raise TypeError(
            f"Post compile hook backend {backend!r} must return only strings"
        )
    return normalized
...
def _lookup_cuda_backend(name: str) -> SchedulingConstructor:
    try:
        return _cuda_backends[name]
    except KeyError as e:
        available = ", ".join(sorted(_cuda_backends)) or "<none>"
        raise KeyError(
            f"Unknown Inductor CUDA backend {name!r}. "
            f"Available CUDA backends: {available}"
        ) from e


def register_dtype_propagation_backend(
    name: str, *, require_output_dtype: bool = True
) -> None:
    """
    Register a backend that uses Inductor's Python dtype/shape propagation.
    ...
    if (
        ...
        raise ValueError(
            f"dtype propagation backend {name!r} is already registered"
        )
    _dtype_propagation_backends[name] = require_output_dtype


def _uses_dtype_propagation(backend: str) -> bool:
    return backend in _dtype_propagation_backends


def _requires_output_dtype(backend: str) -> bool:
    return _dtype_propagation_backends.get(backend, False)


def register_constexpr_syntax(name: str, suffix: str) -> None:
    """
    Register the argument-list suffix for backend compile-time constants.
    ...
    if (
        ...
        raise ValueError(f"constexpr syntax for backend {name!r} is already registered")
    _constexpr_syntaxes[name] = suffix


def _lookup_constexpr_syntax(name: str) -> str:
    try:
        return _constexpr_syntaxes[name]
    except KeyError as e:
        available = ", ".join(sorted(_constexpr_syntaxes)) or "<none>"
        raise KeyError(
            f"Unknown constexpr syntax backend {name!r}. "
            f"Available constexpr syntax backends: {available}"
        ) from e
...
def init_backend_registration() -> None:
    ...
    register_cuda_backend("triton", CUDACombinedScheduling)
    register_cuda_backend("halide", HalideScheduling)
    register_cuda_backend("pallas", PallasScheduling)
    if get_scheduling_for_device("cuda") is None:
        register_backend_for_device(
            "cuda",
            lambda scheduling: _lookup_cuda_backend(config.cuda_backend)(scheduling),
            PythonWrapperCodegen,
            CppWrapperGpu,
            WrapperFxCodegen,
        )
...
class ArgName:
    name: str
    # is_constexpr=True is used to attach backend-specific constexpr syntax
    # into the argument list, such as " : tl.constexpr" for Triton.
    is_constexpr: bool = False
    backend: str = "triton"

    def full_name(self) -> str:
        if self.is_constexpr:
            return f"{self.name}{_lookup_constexpr_syntax(self.backend)}"
        return self.name

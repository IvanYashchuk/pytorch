...
class OutputGraph(OutputGraphCommon):
    ...
    def __init__(
        ...
        # Diagnostic-only callable installed into generated bytecode and carried
        # on GuardedCode/CacheEntry. Normal eval-frame lookup still executes
        # the guarded bytecode path.
        self.diagnostic_stable_callable: Callable[..., Any] | None = None
    ...
    def compile_and_call_fx_graph(
        ...
        with torch._guards.TracingContext.clear_frame():
            ...
            if specializations := old_fake_mode.shape_env.specializations:
                self.diagnostic_stable_callable = None
                ...
                self.diagnostic_stable_callable = (
                    compiled_fn
                    if self.package is None
                    and not self.torch_function_subclass_inlined
                    and not has_user_objects()
                    else None
                )

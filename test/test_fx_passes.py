...
class MockOperatorSupport(OperatorSupport):
    def is_node_supported(self, submodules, node: torch.fx.Node) -> bool:
        return (node.op == "call_function" and
                node.target in {operator.add, operator.getitem, torch.ops.aten.view, torch.ops.aten.permute})
...
class TestFXGraphPasses(JitTestCase):
    ...
    @parametrize("fn, expected_partition, bookend_non_compute_pass", [
        ...
        (TestPartitionFunctions.forward14, [["permute_1", "add_1", "add"]], True),
        (TestPartitionFunctions.forward14, [['add_1', 'add', 'permute_1', 'view', 'permute_2', 'permute_3', 'permute']], False),
